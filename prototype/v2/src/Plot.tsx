...
export function Plot(p:Props){
 ...
 const count=rows.length;const rh=(h-(q.residual?144:0))/count;if(q.residual)rows.push({id:q.method,label:`Reference와 차이 · Output − Reference · 표시 ×${q.gain||1}`,color:'#c0ced7'});g.font='11px system-ui';const labelWidth=Math.max(...amplitudeTicks(q.amplitude,q.residual?q.gain||1:1).concat(amplitudeTicks(q.amplitude)).map(t=>g.measureText(t.label).width));const left=Math.max(72,labelWidth+26),right=w-18,pw=right-left;const pts=visiblePoints(t,q.span,q.data.fs,q.data.n,q.mode,q.inspect,q.soft).filter(p=>p.index>=q.data.offset&&p.index<q.data.offset+q.data.traces.input.length);
 rows.forEach((row,r)=>{const isDiff=q.residual&&r===count;const top=r*rh+36,bottom=isDiff?top+72:(r+1)*rh-30,ph=bottom-top;
 g.textAlign='left';g.textBaseline='alphabetic';g.font='11px system-ui';g.fillStyle='#acc2cd';g.fillText('[mV]',10,r*rh+22);g.font='600 14px system-ui';g.fillStyle=row.color;g.fillText(row.label,left,r*rh+22);
 g.font='11px system-ui';g.fillStyle='#acc2cd';g.textAlign='right';g.textBaseline='middle';for(const tick of amplitudeTicks(q.amplitude,isDiff?q.gain||1:1))g.fillText(tick.label,left-10,top+tick.fraction*ph);g.textAlign='left';g.textBaseline='alphabetic';
