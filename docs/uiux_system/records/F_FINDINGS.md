...
## F-015. 레퍼런스 대비 품질 격차는 층으로 나뉜다 — 수치·재료·타이밍·렌더 함정은 규칙으로, 룩은 나란히 캡처 루프로, 구성·취향만 사람 판단으로

| | |
|---|---|
| 상태 | 확인됨 — 충실도 게이트 제안(P6)의 근거 |
| 발견 | 2026-09-25 REF-003·REF-004 재현 spike, 연구실 코너 v1→v3 `[캡처]` `[테스트]` |
| 영향 | "피드백 반복만이 답"이라는 가정을 버리고, 사용자에게 보이기 전 단계에서 격차 대부분을 줄이는 순서를 정할 수 있다 |
| 연결 | F-010, F-013, F-014, `handoffs/REFERENCE_REPRO_AND_QUALITY_GAP_2026-09-25.md` §4 |

### 발단 — 무엇이 이상해 보였나
사용자 질문 `[대화]`: "레퍼런스에 비해서 시안의 퀄리티가 떨어지는 이유는 뭐고, 해결하려면 어떤 방법이 적절해? 네가 말한 것처럼 결과물을 보면서 피드백하는 수밖에 없어? 아니면 규칙을 따로 정해놓으면 되는거야?" 앞선 보고(§20)에서 AI는 "아트 디렉션 반복 필요"라고만 답했다 `[대화]`.

### 먼저 의심한 것과 배제 방법
"품질 = 사람 눈 판단"이라 규칙화가 어렵다고 보았다 `[추론]`. REF-003·004를 효과 카드 수치 + 외부 고해상도 재료로 재현했더니, 사용자 피드백 없이 한 세션 안에 레퍼런스 가까이 갔다 `[캡처]`. AI가 스스로 고친 횟수는 지구 노출 2회, 지도 명암 3회, 구름 조명 3회, 구도 1회였다 `[런타임]`.

### 결정적 근거
격차를 층으로 나누면 ① 메커니즘 수치 ② 재료 ④ 모션 타이밍 ⑥ 렌더 함정은 사전에 수치로 확인할 수 있고, ③ 룩(색·노출·대비)은 레퍼런스 프레임과 나란히 캡처하는 AI 루프로 대부분 줄었으며, ⑤ 구성·내용(강한 피사체, 카피)만 이번에도 남았다 `[캡처]`.

### 조치와 검토한 대안
- 사용자 피드백 반복만 — 비용이 사용자에게 몰린다.
- 규칙만 — 룩·구성은 규칙으로 닫히지 않는다.
- **규칙(①②④⑥) + AI 나란히 캡처 루프(③) + 사용자 판단(⑤·최종)** — 충실도 게이트로 제안, 사용자 결정 대기(P6).

### 놓쳤다면
시안마다 사용자가 "뭔가 싸 보인다"를 말로 설명하는 반복에 시간을 쓰고, 원인이 재료·수치인지 취향인지 구분하지 못했을 것이다.

### 일반화
품질 격차를 **층으로 분해해 층마다 다른 도구**(수치 계약, 재료 스펙, 나란히 캡처, 사람 판단)를 붙인다. 사람에게는 규칙과 루프로 닫히지 않은 차이만 묻는다.

## F-016. 톤을 눈으로 맞추면 틀린다 — 레퍼런스가 "더 밝아 보인" 이유는 밝기가 아니라 대비·채도였다

| | |
|---|---|
| 상태 | 확인됨 — P6 충실도 게이트 4번(자동 수치 비교)의 실증 |
| 발견 | 2026-09-25 REF-002 다락방 책장 재구성 `[캡처]` `[런타임]` |
| 영향 | 룩 조정 층(F-015 ③)을 AI가 스스로 수렴시킬 수 있는 구체 방법 — 밝기 분위(p5/p50/p95)·채도·난색을 레퍼런스 프레임과 비교 |
| 연결 | F-015, `handoffs/ATTIC_BOOKSHELF_STUDY_2026-09-25.md` §3 |

### 발단 — 무엇이 이상해 보였나
레퍼런스 녹화 프레임과 나란히 보았을 때 우리 화면이 "더 어둡고 칙칙하다"고 AI가 판단했다 `[대화]` `[추론]`.

### 먼저 의심한 것과 배제 방법
노출을 1.55 → 1.95로 올렸지만 차이가 줄지 않았다. 두 프레임의 밝기 분포를 재 보니 레퍼런스의 중간톤(p50 54)은 오히려 우리(75)보다 **어두웠고**, 채도(0.49 대 0.33)와 난색(R−B 41 대 30)이 높았다 `[런타임]`. "밝아 보임"은 높은 대비에서 온 착시였다.

### 결정적 근거
수치를 목표로 색보정을 3회 조정하자(1회차는 채도·난색 과보정) 평균 밝기 74.7(목표 75.2), 채도 0.469(0.494), 난색 43.5(40.8)로 수렴했고, 나란히 본 인상도 가까워졌다 `[캡처]`.

### 조치와 검토한 대안
- 눈대중 반복 — 방향 자체가 틀릴 수 있다(이번 1차 조정).
- 사용자에게 매번 묻기 — 비용이 사용자에게 간다.
- **레퍼런스 프레임과 같은 장면·같은 영역의 분포 비교 → 파라미터화한 색보정 조정** — 채택(P6 제안에 포함).

### 놓쳤다면
노출만 계속 올려 화면이 뿌옇게 떠 버리고, "레퍼런스 느낌이 안 난다"는 피드백이 반복됐을 것이다.

### 일반화
룩을 맞출 때는 **먼저 재고 나서 고친다.** 밝기 분위·채도·색온도처럼 인상에 영향을 주는 값을 레퍼런스와 나란히 수치로 비교하고, 색보정 파라미터를 URL·상수로 노출해 반복을 싸게 만든다.
