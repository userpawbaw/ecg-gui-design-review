...
export function Plot(p:Props){
 ...
 const sig=[t,q.mode,q.span,q.inspect,q.amplitude,q.reference,q.method,q.preview,q.comparison,q.data.scene.id,q.soft,q.residual,q.gain,rect.width,rect.height,dpr].join('|');
 ...
 const count=rows.length;const rh=(h-(q.residual?104:0))/count;if(q.residual)rows.push({id:q.method,label:`Reference와 차이 · Output − Reference · 표시 ×${q.gain||1}`,color:'#c0ced7'});const left=64,right=w-18,pw=right-left;const pts=visiblePoints(t,q.span,q.data.fs,q.data.n,q.mode,q.inspect,q.soft);
 ...
 const y=(i:number)=>top+ph/2-(residual?(a[i]-q.data.traces.clean[i])*(q.gain||1):a[i])/q.amplitude*ph/2;
