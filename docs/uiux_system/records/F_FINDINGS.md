...
## F-017. 효과를 1–2장으로 확인하면 "구간 중에만 생기는" 실패를 놓친다 — REF-003 경로는 12장에서 결함 3개가 드러났다

| | |
|---|---|
| 상태 | 확인됨 — D-019 프레임 수 규칙의 근거 |
| 발견 | 2026-09-26 REF-003 경로 재검토, 사용자 지적 후 `[대화]` `[캡처]` |
| 영향 | 레퍼런스 대조와 자체 QA의 최소 단위를 "효과 구간 전체 균등 캡처"로 바꾼다 |
| 연결 | F-015, D-019, `handoffs/REF003_ROUTE_FIX_COMPONENTS_ASSETS_2026-09-26.md` §2 |

### 발단 — 무엇이 이상해 보였나
사용자가 "표시기가 지나온 지점을 알려주고 있지 않다 … 여러 장을 확인하지 않아서 그런 느낌. 한 대여섯장 이상은 확인해봐야"라고 지적했다 `[대화]`. 이전 QA는 경로 구간의 시작·중간 캡처 1–2장과 진행률 숫자(0 → 97 %)만 보았다(`REFERENCE_REPRO_AND_QUALITY_GAP_2026-09-25.md` §6) `[커밋]`.

### 먼저 의심한 것과 배제 방법
색 대비만 부족하다고 보고 선 색을 고치려 했다. 레퍼런스 12장과 우리 12장을 같은 스크롤 비율로 나란히 놓자 결함이 세 개였다: ① 지난/남은 경로 밝기가 거의 같음, ② 4번째 프레임부터 표시기가 화면 밖으로 나감(고정 트리거 구간과 지도 parallax 불일치), ③ 수정 ① 뒤 청록 선이 두 토막으로 갈라짐(`vector-effect: non-scaling-stroke` + DrawSVG) `[캡처]`.

### 결정적 근거
②는 시작·끝 장에서는 정상이고 중간 구간에서만 생긴다. ③은 ①을 고친 뒤에야 나타났다. 수정 후 12장 표시기 y 수열이 가운데 구간에서 468–500 px(화면 900 px)로 유지됨을 확인했다 `[테스트]`.

### 조치와 검토한 대안
- 사람이 매번 영상을 보고 피드백 — 비용이 사용자에게 간다.
- 1–2장 + 진행률 숫자 — 이번 실패의 원인.
- **효과 구간 균등 6장 이상(권장 12장) 나란히 대조 + 핵심 좌표 수열 기록** — 채택(D-019). 시간 루프 효과(소나)는 같은 방식으로 시간축 12장.

### 놓쳤다면
"경로가 그려진다"는 사실만 남고, 레퍼런스의 핵심 정보(얼마나 왔는가)와 따라오는 표시기라는 wow가 빠진 채 파이프라인 결정으로 넘어갔을 것이다.

### 일반화
움직이는 효과는 한 장면이 아니라 **구간**이다. 수정 하나가 다른 결함을 드러낼 수 있으므로 고칠 때마다 같은 구간을 다시 전부 캡처한다.

## F-018. 연속 프레임 스크롤은 "받기"보다 "디코딩"이 병목이다 — 720p 240장을 모두 풀면 약 885 MB

| | |
|---|---|
| 상태 | 확인됨 — 플레이어 설계에 반영(디코딩 창) |
| 발견 | 2026-09-26 AI 영상 스크롤 플레이어 설계 검토 `[코드]` `[테스트]` |
| 영향 | 영상·연속 이미지 스크롤 구간의 메모리 예산과 로딩 전략(22번 §4 #6–7) |
| 연결 | REF-004 EFX-004-06, D-023, `22_AI_VIDEO_SCROLL_PIPELINE.md` |

### 발단 — 무엇이 이상해 보였나
첫 플레이어는 REF-004의 교훈(먼 프레임 대체 금지)을 지키려고 모든 프레임을 `ImageBitmap`으로 미리 디코딩했다 `[코드]`. 압축 파일은 12–13 MB라 가벼워 보였다.

### 먼저 의심한 것과 배제 방법
전송량만 보면 문제없다고 판단했다. 디코딩된 크기를 계산하니 1280×720×4바이트×240장 ≈ 885 MB, 4K면 ≈ 3.5 GB였다 `[추론]`. 전시 PC GPU(P2 미정)에서 위험하다.

### 결정적 근거
압축 파일은 전부 받되 재생 위치 ±24장만 디코딩하고 먼 프레임은 해제하도록 바꾸자, 동시 보유 최대 73장(≈ 270 MB @720p), 연속 휠 스크롤 42칸에서 디코딩 대기 0회, 12구간 순간 이동에서도 2–3틱 직전 화면 유지뿐이었다 `[테스트]`.

### 조치와 검토한 대안
- 전부 디코딩 — 메모리 초과 위험.
- 필요할 때만 받기(REF-004 방식) — 가까운 프레임 대체로 튐.
- **전부 받기 + 디코딩 창 + 직전 화면 유지** — 채택.
- all-intra 영상 + WebCodecs — 미시험, 4K에서 다시 검토.

### 놓쳤다면
개발 PC에서는 잘 돌다가 전시 PC나 4K 화면에서 탭이 죽거나 멈췄을 것이다.

### 일반화
연속 이미지·영상 연출은 **압축 크기와 디코딩 크기를 따로** 예산에 넣는다. 디코딩 크기 = 폭 × 높이 × 4 × 동시 보유 장수.

## F-019. REF-002의 "고개 돌림"은 제자리 회전이라 여백 있는 영상 이동으로 거의 정확히 재현된다 — 섞기는 잔상을 만든다

| | |
|---|---|
| 상태 | 확인됨 — 22번 §5.2의 첫 판 판단을 정정 |
| 발견 | 2026-09-26, 사용자 반박 후 다락방 장면 렌더 비교 `[대화]` `[테스트]` |
| 영향 | 영상 경로의 마우스 시선 방식, 브리프 여백 요구, V0 판정 기준 |
| 연결 | D-023, D-026, REF-002 EFX-002-01, `22_AI_VIDEO_SCROLL_PIPELINE.md` §5.2, `verification/video-idle-headturn-20260926/` |

### 발단 — 무엇이 이상해 보였나
AI는 첫 판에서 "REF-002의 고개 돌림은 실시간 3D라서 가능하고 평면 영상에서는 가려진 부분이 없어 한계"라고 썼다 `[커밋]`. 사용자는 "고개를 돌리면 새로운 부분이 보이는 느낌을 주는 거는 영상을 그렇게 만들어 달라고 하면 가능하지?"라고 반박했다 `[대화]`.

### 먼저 의심한 것과 배제 방법
AI는 시점 변형 영상 여러 개를 마우스로 섞는 방식을 먼저 떠올렸다. 다락방 장면에 캡처 훅을 넣어 진짜 회전·이동 렌더, 넓은 화각(여백) 렌더, 실제 깊이를 찍고 흉내 방식과 픽셀 비교했다. 첫 측정은 부호(three.js yaw + = 왼쪽 회전)와 여백 배율 방향이 틀려 흉내가 오히려 나빴고, 빛줄기 디더링·grain이 렌더마다 달라 점수가 잡음에 묻혔다 — 캡처 모드에서 잡음을 고정하고 고친 뒤 다시 쟀다 `[테스트]`.

### 결정적 근거
REF-002의 시선은 ±0.75° **제자리 회전**이라 시차가 없다. 여백 이동(19.6 px)이 진짜 회전 렌더와 평균 오차 4.6/255, 흐림 SSIM 0.972(아무것도 안 하면 18.4, 0.749). 좌우 이동 0.12에서는 평면 이동 5.8 ≈ 실제 깊이 변형 5.9. 좌·우 변형 영상 50 % 섞기는 중간 시점과 16.3으로 거의 개선이 없고 사다리·소품이 이중으로 보였다 `[테스트]`.

### 조치와 검토한 대안
- 변형 영상 섞기 — 잔상, 기각.
- 깊이 맵 시차 — 깊이 차가 작은 장면에서는 이득이 없고 경계 늘어짐.
- **여백 이동을 기본**으로, 브리프에 여백 6 % 이상 요구 — 채택. 진짜 시차가 핵심이면 실시간 3D.

### 놓쳤다면
REF-002형 연출을 모두 비싼 3D 경로로 보내거나, 잔상이 생기는 변형 영상 섞기를 제품에 넣었을 것이다.

### 일반화
"영상으로 되나"를 판단하기 전에 효과의 **물리**(회전인가 이동인가)를 먼저 가린다. 회전은 이미지 변환으로 정확히 되고, 이동은 깊이 차에 비례해 어려워진다. 비교 렌더는 잡음(디더링·grain)을 고정하고 잰다.

## F-020. "멈춰도 재생" 방식은 반복 영상에서만 성립하고, 영상 위 생명감 층은 영상에 구워진 같은 요소와 충돌한다

| | |
|---|---|
| 상태 | 확인됨 — D-026의 근거 |
| 발견 | 2026-09-26 정지 상태 시험(`qa-idle.mjs`) `[테스트]` |
| 영향 | 정지 생명감 방식 선택, 브리프 요구(입자 없이·완전 반복) |
| 연결 | D-026, REF-001 moto 지구(F-012), REF-003 EFX-003-03, `22_AI_VIDEO_SCROLL_PIPELINE.md` §10 |

### 발단 — 무엇이 이상해 보였나
사용자가 AI의 지적("스크롤을 멈추면 영상도 멈춘다")에 두 방식을 제안했다: 1) 영상 위에 먼지·빛·반복 클립 층, 2) 멈춰도 천천히 재생되다가 스크롤로 가속 `[대화]`.

### 먼저 의심한 것과 배제 방법
두 방식 모두 플레이어에 넣고 반복 영상(지구 한 바퀴)과 반복이 아닌 영상(다락방 하강)에 각각 적용해 재생 위치를 시간별로 기록했다.

### 결정적 근거
2번 — 지구: 정지 4.0 fps, 스크롤 중 45 fps, 멈춘 뒤 4.6 fps로 moto 지구처럼 동작. 다락방: 정지 중 영상이 스크롤 위치보다 앞서 가고(9.5 → 21.5), 스크롤 25 지점에서 영상이 끝(47)에 도달해 남은 구간 동안 멈췄다. 1번 — 정지 중 변화 픽셀 0 → 0.98 %로 살아나지만, 영상에 이미 찍힌 먼지는 멈춘 채 남아 움직이는 먼지와 섞였다 `[테스트]`.

### 조치와 검토한 대안
- 2번을 모든 영상에 — 카메라 경로에서 위치 어긋남·조기 종료, 기각.
- **카메라 경로 = 1번, 반복 대상 = 2번(+1번)**, 브리프에 "입자 없이"와 "완전 반복" 요구, 입고 QA에 반복 이음매 검사 — 채택.

### 놓쳤다면
카메라 하강 영상이 스크롤보다 먼저 끝나 구간 절반이 멈춘 화면이 되거나, 멈춘 먼지와 움직이는 먼지가 섞인 어색한 정지 화면이 나왔을 것이다.

### 일반화
시간으로 움직이는 층을 영상에 더할 때는 **같은 요소를 영상과 코드 중 한쪽에만** 둔다. 재생 위치를 스크롤에서 떼어 낼 수 있는 것은 위치 의미가 없는(반복·회전) 움직임뿐이다.
