...
const inputPaths=['prototype/index.html','prototype/style.css','data/bank.js','data/extension.js','prototype/core.js','prototype/app.js'];
const receipt={file:'dist/ecg-signal-studio.html',bytes:Buffer.byteLength(html),sha256:crypto.createHash('sha256').update(html).digest('hex'),builtFrom:'prototype/ + data/bank.js + data/extension.js',externalAssets:0,inputs:inputPaths.map(p=>({path:p,sha256:crypto.createHash('sha256').update(fs.readFileSync(path.join(root,p))).digest('hex')}))};
