...
- User authorized direct local Playwright and isolated branch push. Captured 1920×1080 A/B Chromium screenshots and 17.3-second A/B/Compare video. Directly compared both attached Beta images, corrected white inherited header, missing Korean glyphs, plot amplitude, B heading/card overlaps and landscape seam. `prototype/v2/design-qa.md` records remaining intentional differences; 1920×1080 prototype visual QA PASS.
...
- Browser evidence: B comparison lens keyboard 50→52%; B→Compare `d1-mixed-10`/`M08` 11.133→11.333 s; A→B same scene/method; accelerated A handoff 17.766→20.009 s. Reduced motion enters paused and handoff succeeds. RAF intervals (headless, recording disabled) B median/p95 16.7/16.7 ms (299 samples), A 16.7/33.4 ms (527 samples); these are frame intervals rather than paint cost. Console/page errors 0. No WebGL, merge or data regeneration.

## 2026-09-25 — Attract vNext scratch recovery and final recapture

- The 2026-09-24 scratch checkout was pruned during an interrupted authenticated push. Reconstructed all code/document blobs from GitHub object storage on a separate remote checkpoint branch; restored the exact v2.2.1 ZIP (SHA-256 `9070d4aa…22ee0fa1e98ed`) from persistent user files without regenerating data. Recreated a dark mountain asset after the previous local generated file was lost.
- CDN Playwright browser ZIP was empty. Installed Chromium 153 from the npm package cache into a temporary runtime path; the dependency is not added to the project. Retested A/B at 1920×1080 and recorded a new 17.3-second browser video. Current screenshots, recording and `runtime-results.json` supersede the 2026-09-24 paths. Build, 15/15 targeted assertions and 37 records pass. Browser console 0 errors; reduced motion handoff passes; headless RAF B 16.7/16.7ms, A 16.7/33.4ms median/p95. No merge.
