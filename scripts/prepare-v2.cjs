...
const parse=s=>{
 ...
 if(start<=0||end<start)throw Error('ECG bank assignment is malformed.');
...
const methodsPath=path.join(root,'prototype/v2/src/methods.json');
if(!fs.existsSync(methodsPath))fs.writeFileSync(methodsPath,JSON.stringify(metadata,null,2));
