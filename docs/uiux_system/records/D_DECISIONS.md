...
| 상태 | **채택 — 2026-09-26 사용자 확정** ("AI 영상 파이프라인 확정하고 P1, P5, P6 결정으로 넘어가자" `[대화]`). 자체 시험 2차(F-019·F-020·D-025·D-026) 반영. 운영 값(피드백 3회 상한, 여백 6 %, 입고 QA 기준)은 첫 실제 AI 영상 시험에서 조정 |
...
## D-027. (P1) 연출 제작 파이프라인을 HIGH·MEDIUM 구역에 정식 채택하고, HIGH 구역에서 WebGL을 허용한다

| | |
|---|---|
| 시점 | 2026-09-26, 사용자 결정 "HIGH·MEDIUM 전체" (P1 선택지 질문) `[대화]` |
| 상태 | 채택 |
| 연결 | F-010, F-014, F-015, D-016(복구 브랜치), D-017–D-026, `25_EFFECT_PRODUCTION_PIPELINE.md`, `handoffs/EFFECT_PRODUCTION_PIPELINE_AUDIT_2026-09-25.md` §7 |
| CASE | CASE-006 |

### 갈림길
2026-09-25 P1(파이프라인 채택)은 "레퍼런스 2–3개를 더 분석한 뒤 결정"으로 보류됐다 `[대화]`. 그 뒤 레퍼런스 4개 분석, 재현 spike, 다락방, AI 영상 경로를 거치며 원 제안 7계층 중 5개가 이미 쓰이고 있었다 `[커밋]`. 남은 것은 적용 범위와 스택 기준(D-016 개정)이었다.

### 검토한 선택지
1. **HIGH·MEDIUM 전체 필수**, HIGH에서 WebGL 허용, MEDIUM은 기본 DOM/Canvas 2D/SVG. 2. 레퍼런스 재현에만 적용(제품 미적용, D-016 유지). 3. HIGH 구역에만 필수.

### 고른 것과 근거
1번(사용자 선택, AI 권장과 같음). 품질 격차의 원인이 발상이 아니라 렌더링 상한·재료·검증 부재였고(F-010, F-015), 재현 spike에서 이 순서가 실제로 격차를 줄였다(F-016, F-017). MEDIUM에서 WebGL을 기본으로 두지 않는 이유는 F-014 — 경로·소나·마스크 같은 wow는 DOM/SVG로 충분하고 가볍다.

### 버린 것과 이유
2번 — 학습 결과가 제품에 들어가지 않는다. 3번 — 결과 요약·metric reveal 같은 MEDIUM 연출이 같은 품질 격차를 반복한다.

### 되돌려야 하는 조건
P2(전시 PC) 실측에서 WebGL 구간이 §5 예산을 지키지 못하면 HIGH의 WebGL 허용 범위를 줄인다. MEDIUM 작업에서 파이프라인 단계 비용이 두 번 연속 결과 개선보다 크면 MEDIUM을 선택 적용으로 낮춘다.

## D-028. (P5) 3D·스크롤 연출은 vanilla 엔진 모듈로 만들고 React는 마운트와 상태 전달만 맡는다

| | |
|---|---|
| 시점 | 2026-09-26, 사용자 결정 "vanilla 엔진 + React 마운트" `[대화]` |
| 상태 | 채택 |
| 연결 | D-021, REF-004(r3f 사이트를 vanilla로 재현), `25_EFFECT_PRODUCTION_PIPELINE.md` §4, 감사 문서 §13 |
| CASE | CASE-006 |

### 갈림길
제품 v2.2.1은 React 19다 `[코드]`. 3D를 React Three Fiber로 쓸지, vanilla three 엔진을 React가 붙이는 구조로 할지.

### 검토한 선택지
1. **vanilla 엔진 + React 마운트**(인터페이스: setState/setProgress/setData/on/resize/dispose). 2. React Three Fiber + drei. 3. 첫 제품 구현 때 A/B로 결정.

### 고른 것과 근거
1번(사용자 선택, AI 권장과 같음). 레퍼런스 4의 r3f 구조(렌더 타깃 4장 + 합성)를 vanilla로 그대로 재현했고 `[테스트]`, 레퍼런스·튜토리얼 코드 대부분이 vanilla라 이식이 쉽다. 컴포넌트 A/B(D-021)에서 React·motion 층을 더하면 번들이 2배 이상이었고 핵심 로직은 어차피 직접 썼다. 하나의 timeline으로 3D·DOM·전환을 프레임 단위로 맞추는 연출이 목표라 통제권이 중요하다.

### 버린 것과 이유
2번 — drei 편의는 크지만 매 프레임 state 흐름의 성능 위험과 레퍼런스 코드 번역 비용, three/r3f/drei 버전 맞춤 부담. 필요한 기능(환경맵, glTF, 후처리)은 공식 addon에 있다. 3번 — 결정 지연으로 엔진 인터페이스가 정해지지 않는다.

### 되돌려야 하는 조건
제품 통합에서 어댑터 코드(상태 동기화·수명주기)가 엔진 코드보다 커지거나, drei 전용 기능(예: 특정 재질·텍스트)이 두 번 이상 필요해지면 해당 구간만 r3f를 시험한다.

## D-029. (P6) 충실도 게이트 6항목을 채택하고 AI 자기 수정은 3회까지 한다

| | |
|---|---|
| 시점 | 2026-09-26, 사용자 결정 "전 항목 채택, 수정 3회" `[대화]` |
| 상태 | 채택 |
| 연결 | F-015, F-016, F-017, D-019, D-023, `25_EFFECT_PRODUCTION_PIPELINE.md` §6, `handoffs/REFERENCE_REPRO_AND_QUALITY_GAP_2026-09-25.md` §4.1 |
| CASE | CASE-006 |

### 갈림길
품질 격차 분석(F-015)은 격차를 규칙으로 잡을 층(수치·재료·타이밍·렌더 함정)과 캡처 루프로 잡을 층(룩), 사람 판단 층(구성·취향)으로 나눴다. 사용자에게 보이기 전에 AI가 어디까지 스스로 걸러야 하나.

### 검토한 선택지
1. **6항목 전부 필수(G4는 룩 작업일 때), 자기 수정 3회.** 2. 핵심만(나란히 캡처·상태 전수·수정 한도). 3. 6항목, 자기 수정 5회.

### 고른 것과 근거
1번(사용자 선택, AI 권장과 같음). G3은 REF-003에서 1–2장 확인이 놓친 결함 3개를 잡았고(F-017), G4는 눈대중이 틀린 톤 방향을 바로잡았다(F-016). 수정 한도 3회는 AI 영상 피드백 3회와 맞춰 규칙을 하나로 둔다. G1·G2는 비용이 작고 재현 계획 단계에서 이미 쓰는 정보다.

### 버린 것과 이유
2번 — 수치·재료 누락이 뒤늦게 발견되면 재작업이 크다. 3번 — 사용자 대기 시간이 길어지고, 3회 뒤에도 남는 GAP는 대개 취향·구성 층이라 AI가 더 고쳐도 수렴하지 않는다 `[추론]`.

### 되돌려야 하는 조건
3회 안에 GAP가 줄지 않는 작업이 연속되면 한도가 아니라 경로(S2) 선택을 먼저 재검토한다. 게이트 산출물 작성이 작업 시간의 절반을 넘기면 G5 상태 목록을 효과 카드의 상태로 한정한다.
